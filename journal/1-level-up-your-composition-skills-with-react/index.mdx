...
At work, I was tasked on enhancing the <UnknownTerm term="dx">_DX_</UnknownTerm> of this bloated map component that was becoming harder and harder to maintain. On top of that, it wasn't even handling all of their use-cases, so they had to repeat code in multiple views.
...
When I find myself adding flags ☝️ to determine the behavior of a nested component I smell trouble. So I set myself to create the ultimate API for this component that would allow us to toggle layers (logic + template) of the component without adding more props and mixing unrelated logic.
